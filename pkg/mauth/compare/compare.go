...
func (c *Integer) UnmarshalBinary(b []byte) error {
	if len(b) < integerLen {
		return fmt.Errorf("compare: integer compare bytes is %d, expected %d", len(b), integerLen)
	}
	if b[0] > integerOpMax {
		return fmt.Errorf("compare: unknown integer operation %d", b[0])
	}
	c.Op = IntegerOp(b[0])
	c.Int = int64(binary.BigEndian.Uint64(b[1:]))
	return nil
}
...
func (c Integers) MarshalBinary() ([]byte, error) {
	...
	buf := make([]byte, 1+(len(c)*integerLen))
	buf[0] = byte(TypeIntegers)
	for i, e := range c {
		buf[i+1] = byte(e.Op)
		binary.BigEndian.PutUint64(buf[(i*integerLen)+2:], uint64(e.Int))
	}
	return buf, nil
}
...
func (c *Integers) UnmarshalBinary(b []byte) error {
	...
	b = b[1:]
	if len(b)%integerLen != 0 {
		return fmt.Errorf("compare: expected buffer length to be divisible by %d, got %d", integerLen, len(b))
	}
	*c = make(Integers, len(b)/integerLen)
	for i := range *c {
		if err := (*c)[i].UnmarshalBinary(b[i*integerLen:]); err != nil {
			return fmt.Errorf("compare: error decoding element %d: %s", i, err)
		}
	}
	return nil
}
